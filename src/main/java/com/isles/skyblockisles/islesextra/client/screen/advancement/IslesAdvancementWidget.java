...
public class IslesAdvancementWidget {
  ...
  private IslesAdvancementWidget getParent(PlacedAdvancement startNode) {
    ...
    while (current != null) {
      if (current.getAdvancement().comp_1913().isPresent()) { // Display
        return this.tab.getWidget(current.getAdvancementEntry());
      }
  ...
  public void renderLines(DrawContext context, int x, int y, boolean border) {
    if (this.parent != null) {
      ...
      int l = x + this.x + 13;
      int m = y + this.y + 13;
      int color = border ? Colors.BLACK : Colors.WHITE;
      if (border) {
        context.drawHorizontalLine(j, i, k - 1, color);
        context.drawHorizontalLine(j + 1, i, k, color);
        context.drawHorizontalLine(j, i, k + 1, color);
        context.drawHorizontalLine(l, j - 1, m - 1, color);
        context.drawHorizontalLine(l, j - 1, m, color);
        context.drawHorizontalLine(l, j - 1, m + 1, color);
        context.drawVerticalLine(j - 1, m, k, color);
        context.drawVerticalLine(j + 1, m, k, color);
      } else {
        context.drawHorizontalLine(j, i, k, color);
        context.drawHorizontalLine(l, j, m, color);
  ...
  public void renderWidgets(DrawContext context, int x, int y) {
    if (!this.display.isHidden() || this.progress != null && this.progress.isDone()) {
      ...
      AdvancementObtainedStatus status = f >= 1.0f ? AdvancementObtainedStatus.OBTAINED : AdvancementObtainedStatus.UNOBTAINED;
  ...
  public void drawTooltip(DrawContext context, int originX, int originY, float alpha, int x, int y) {
    ...
    boolean shouldFlip = x + originX + this.x + this.width + 26 >= this.tab.getScreen().width;
    ...
    boolean hitsBottom = y - originY - this.y - 26 <= 6 + this.description.size() * this.client.textRenderer.fontHeight;
    float f = this.progress == null ? 0.0f : this.progress.getProgressBarPercentage();
    ...
    int k = this.width - j;
    int l = y + originY + this.y;
    int m = x + (shouldFlip ? originX + this.x - this.width + 26 + 6 : originX + this.x);
    ...
    context.drawGuiTexture(RenderPipelines.GUI_TEXTURED, advancementObtainedStatus3.getFrameTexture(this.display.getFrame()), x + originX + this.x + 3, y + originY + this.y, 26, 26);
    if (shouldFlip) {
      context.drawTextWithShadow(this.client.textRenderer, this.title, m + 5, y + originY + this.y + 9, -1);
      if (text != null) {
        context.drawTextWithShadow(this.client.textRenderer, text, x + originX + this.x - i, y + originY + this.y + 9, Colors.WHITE);
      }
    } else {
      context.drawTextWithShadow(this.client.textRenderer, this.title, x + originX + this.x + 32, y + originY + this.y + 9, -1);
      if (text != null) {
        context.drawTextWithShadow(this.client.textRenderer, text, x + originX + this.x + this.width - i - 5, y + originY + this.y + 9, Colors.WHITE);
      }
    }
    if (hitsBottom) {
      ...
      for (int o = 0; o < this.description.size(); ++o) {
        context.drawText(this.client.textRenderer, this.description.get(o), m + 5, y + originY + this.y + 9 + 17 + o * this.client.textRenderer.fontHeight, -5592406, false);
      }
    }
    context.drawItemWithoutEntity(this.display.getIcon(), x + originX + this.x + 8, y + originY + this.y + 5);
  }
}