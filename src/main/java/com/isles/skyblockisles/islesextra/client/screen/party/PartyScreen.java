...
extends Screen {
    ...
    public PartyScreen() {
        super(Text.translatable("party_managment_screen.islesextra.name"));
    ...
    protected void init() {
        if (this.initialized) {
            ...
            this.playerList = new PartyPlayerListWidget(this, this.client, this.width, this.getPlayerListBottom() - 88, 88, 36);
        }
        ...
        this.blockingButton = this.addDrawableChild(ButtonWidget.builder(BLOCKING_TEXT, ConfirmLinkScreen.opening(this, "https://aka.ms/javablocking")).dimensions(n, m, l, 20).build());
    ...
    public Collection<PlayerListEntry> getFilteredPlayerList() {
        Collection<PlayerListEntry> list = client.getNetworkHandler().getPlayerList();
    ...
    private void updateServerLabel(MinecraftClient client) {
        if (this.client == null) return;
        int i = getFilteredPlayerList().size();
