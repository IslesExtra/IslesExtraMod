...
public class TexturedGui extends Screen {
    ...
    public void init() {
        ...
        assert client != null;
        setGuiData(width, height, defaultOffsetX, defaultOffsetY, textureWidth, textureHeight, new Identifier("islesextra:textures/gui/" + textureName + ".png"));
        ...
        for (JsonElement c : components) {
            ...
            if (component.get("type").getAsString().equalsIgnoreCase("texture")) {
                ...
                Identifier texture = component.has("texture") ? new Identifier("islesextra:textures/gui/" + component.get("texture").getAsString() + ".png") : gui_texture;
    ...
    public boolean mouseClicked(double mouseX, double mouseY, int button) {
        ...
        getElements().forEach(guiElement -> {
            if (guiElement instanceof Interactable element) {
                if (!ScreenUtils.isMouseOver(mouseX, mouseY, element.getScreenX(this), element.getScreenY(this), element.getSizeX(), element.getSizeY())) return;
                element.interact();
            }
        });
        return true;
